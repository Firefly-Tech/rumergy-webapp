...
def get_token():
    ...
    token = requests.post(
        ...
        data={
            "username": f"{config('RPYC_USER')}",
            "password": f"{config('RPYC_PASS')}",
        },
    ).json()
...
def connect_meter(ip, port):
    ...
    if not meter.connect():
        ...
        try:
            ...
            err_meter = requests.get(
                f"http://127.0.0.1:8000/api/meters/?ip={ip}",
                headers={"Authorization": f"Bearer {access_token}"},
            ).json()
            # Assumes unique IP per meter
            err_id = err_meter[0]["id"]
            error_status = requests.patch(
                f"http://127.0.0.1:8000/api/meters/{err_id}/",
                headers={"Authorization": f"Bearer {access_token}"},
                data={"status": "ERR"},
            ).json()
